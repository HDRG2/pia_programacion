...
def agregar_producto(root,productos_menu,main_menu):
  ...
  print("TipoProductos:",tipo_productos)
  nombres_tipos_productos = [tipo[1] for tipo in tipo_productos]
  ...
  nombre_label4.grid(row=5,column=0, pady=(10,0))
  ...
  combo = ttk.Combobox(
    state="readonly",
    values=nombres_tipos_productos
    )
  combo.place(x=90,y=145)
  combo.bind('<<ComboboxSelected>>',mostrar_seleccion)
  
  mapa_tipo_producto ={
    tipo[1]: tipo[0] for tipo in tipo_productos
  }
